...
namespace Config {
  ...
  const uint8_t S3 = 11;              // Pin selezione MUX bit 3

  // Bottoni
  const uint8_t BTN_UP = 7;           // Bottone incremento shift
  const uint8_t BTN_DOWN = 6;         // Bottone decremento shift
...
inline int toMidi(int v) {             // Funzione inline per convertire valori analogici
  return map(v, 0, 1023, 0, 127);     // Mappa da 0-1023 (ADC) a 0-127 (MIDI)
}
...
namespace Midi {
  void sendCC(uint8_t cc, uint8_t val) {     // Invio Control Change
    midiEventPacket_t ev = {0x0B, 0xB0, cc, val}; // Pacchetto MIDI
